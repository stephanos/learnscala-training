...
abstract class Test_for_a[T: TypeTag] extends BaseTest[T]() {
              ...
              mustReturnList() {
                l =>
                  "must have titles of books with author 'Martin'" ! {
                    l.toSeq must containAllOf(List("Programming In Scala", "Patterns of Enterprise Application Architecture"))
                  }
              }
            }
        }
              ...
              mustReturnList() {
                l =>
                  "must have ISBNs of books where author's lastname starts with 'S'" ! {
                    l.toSeq must containAllOf(List("3868020640", "0321127420"))
                  }
              }
            }
        }
              ...
              mustReturnList() {
                l =>
                  "must have list of title and prices where 'Scala' is in the title" ! {
                    l.toSeq must containAllOf(List(("Programming In Scala" -> 36.95f), ("Programming Scala" -> 27.95f),
              ...
              mustReturnList() {
                l =>
                  "must have list of title that are no longer available" ! {
                    l.toSeq must containAllOf(List("Java Virtual Machine", "Patterns of Enterprise Application Architecture"))
                  }
              }
            }
        }
              ...
              mustReturnList() {
                l =>
                  "must have list of title that are out of stock" ! {
                    l.toSeq must containAllOf(List("Programming In Scala"))
                  }
              }
            }
        }
